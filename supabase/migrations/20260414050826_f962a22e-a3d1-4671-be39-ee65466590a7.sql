...
-- Add composite indexes for faster JOINs
CREATE INDEX IF NOT EXISTS idx_stock_item_type ON stock(item_id, type_store);
CREATE INDEX IF NOT EXISTS idx_sales_by_week_id18_type ON sales_by_week(id18, type_store);
CREATE INDEX IF NOT EXISTS idx_minmax_item_type ON minmax(item_id, type_store);

-- Optimized function using temp table approach
...
AS $function$
BEGIN
  -- Create temp table for base items to avoid repeated subqueries
  CREATE TEMP TABLE IF NOT EXISTS _srr_base ON COMMIT DROP AS SELECT 1 WHERE false;
  TRUNCATE _srr_base;
  DROP TABLE _srr_base;
  
  CREATE TEMP TABLE _srr_base ON COMMIT DROP AS
  ...
  LEFT JOIN LATERAL (
    ...
    FROM vendor_master vm2 WHERE vm2.vendor_code = b.vendor_code LIMIT 1
  ) vm ON true
  LEFT JOIN LATERAL (
    SELECT rs.final_rank FROM rank_sales rs
    WHERE rs.item_id = b.sku ORDER BY rs.created_at DESC LIMIT 1
  ) rd ON true
  LEFT JOIN LATERAL (
    SELECT
    ...
    FROM minmax m WHERE m.item_id = b.sku
  ) mm ON true
  LEFT JOIN LATERAL (
    SELECT
    ...
    FROM stock s WHERE s.item_id = b.sku
  ) st ON true
  LEFT JOIN LATERAL (
    SELECT
    ...
    FROM sales_by_week sw WHERE sw.id18 = b.sku
  ) sl ON true
  LEFT JOIN LATERAL (
    SELECT COALESCE(pc2.moq, 1) AS moq, COALESCE(pc2.po_cost, 0) AS po_cost, COALESCE(pc2.po_cost_unit, 0) AS po_cost_unit
    FROM po_cost pc2 WHERE pc2.item_id = b.sku ORDER BY pc2.created_at DESC LIMIT 1
  ) pc ON true
  LEFT JOIN LATERAL (
    SELECT COALESCE(SUM(o.po_qty), 0) AS total_po_qty
    FROM on_order o WHERE o.sku_code = b.sku
  ) oo ON true
  ORDER BY b.vendor_code, b.sku;
END;
$function$;
