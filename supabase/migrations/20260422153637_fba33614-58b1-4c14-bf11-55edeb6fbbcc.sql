...
AS $function$
  ...
  mm AS (
    SELECT
      ...
      SUM(CASE WHEN UPPER(type_store) IN ('KOKKOK','KOKKOK-FC') THEN COALESCE(min_val,0) END) AS min_kokkok,
      SUM(CASE WHEN UPPER(type_store) IN ('KOKKOK','KOKKOK-FC') THEN COALESCE(max_val,0) END) AS max_kokkok,
  ...
  st AS (
    SELECT item_id,
      ...
      SUM(CASE WHEN UPPER(type_store) IN ('KOKKOK','KOKKOK-FC') THEN COALESCE(quantity,0) END) AS stock_kokkok,
  ...
  sw AS (
    SELECT COALESCE(item_id, id18) AS item_id,
      ...
      SUM(CASE WHEN UPPER(type_store) IN ('KOKKOK','KOKKOK-FC') THEN COALESCE(avg_day,0) END) AS avg_sales_kokkok,
