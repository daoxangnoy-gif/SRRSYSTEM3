...
-- Server-side function: fetch SRR data with all JOINs and aggregations
-- Returns only filtered rows with all fields pre-computed
...
RETURNS TABLE (
  ...
  stock_dc numeric,
  stock_jmart numeric,
  stock_kokkok numeric,
  stock_udee numeric,
  avg_sales_jmart numeric,
  avg_sales_kokkok numeric,
  avg_sales_udee numeric,
  moq numeric,
  po_cost numeric,
...
BEGIN
  ...
  WITH filtered_vendors AS (
    SELECT vm.vendor_code AS vc, vm.spc_name AS sn, vm.order_day AS od,
  ...
  base_items AS (
    ...
    WHERE dm.sku_code IS NOT NULL
      ...
      AND (
        ...
        OR dm.vendor_code IN (SELECT vc FROM filtered_vendors)
      )
  ...
  rank_data AS (
    ...
    FROM rank_sales rs
    WHERE rs.item_id IN (SELECT bi.sku_code FROM base_items bi)
  ...
  SELECT
    ...
    COALESCE(fv.sn, '') AS spc_name,
    COALESCE(fv.od, '') AS order_day,
    COALESCE(rd.final_rank, '') AS rank_sales,
    COALESCE(fv.lt, 0) AS leadtime,
    COALESCE(fv.oc, 0) AS order_cycle,
    COALESCE(mma.min_jmart, 0) AS min_jmart,
    COALESCE(mma.max_jmart, 0) AS max_jmart,
    COALESCE(mma.min_kokkok, 0) AS min_kokkok,
    COALESCE(mma.max_kokkok, 0) AS max_kokkok,
    COALESCE(mma.min_udee, 0) AS min_udee,
    COALESCE(mma.max_udee, 0) AS max_udee,
    COALESCE(sa.stock_dc, 0) AS stock_dc,
    COALESCE(sa.stock_jmart, 0) AS stock_jmart,
    COALESCE(sa.stock_kokkok, 0) AS stock_kokkok,
    COALESCE(sa.stock_udee, 0) AS stock_udee,
    ROUND(COALESCE(sla.avg_jmart, 0)::numeric, 4) AS avg_sales_jmart,
    ROUND(COALESCE(sla.avg_kokkok, 0)::numeric, 4) AS avg_sales_kokkok,
    ROUND(COALESCE(sla.avg_udee, 0)::numeric, 4) AS avg_sales_udee,
...
CREATE INDEX IF NOT EXISTS idx_data_master_filters ON data_master (packing_size_qty, stock_unit_flag, buying_status, item_type, product_owner);
CREATE INDEX IF NOT EXISTS idx_data_master_vendor_code ON data_master (vendor_code);
CREATE INDEX IF NOT EXISTS idx_data_master_sku_code ON data_master (sku_code);
CREATE INDEX IF NOT EXISTS idx_rank_sales_item_id ON rank_sales (item_id);
CREATE INDEX IF NOT EXISTS idx_minmax_item_id ON minmax (item_id);
CREATE INDEX IF NOT EXISTS idx_stock_item_id ON stock (item_id);
CREATE INDEX IF NOT EXISTS idx_sales_by_week_id18 ON sales_by_week (id18);
CREATE INDEX IF NOT EXISTS idx_po_cost_item_id ON po_cost (item_id);
CREATE INDEX IF NOT EXISTS idx_on_order_sku_code ON on_order (sku_code);
CREATE INDEX IF NOT EXISTS idx_vendor_master_vendor_code ON vendor_master (vendor_code);
CREATE INDEX IF NOT EXISTS idx_vendor_master_spc_order ON vendor_master (spc_name, order_day);
