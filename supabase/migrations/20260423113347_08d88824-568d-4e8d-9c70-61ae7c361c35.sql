...
AS $function$
  ...
  -- ship_to map: clean ship_to (left of ':') -> store_type.store_name (canonical store key)
  ship_map AS (
  ...
  -- on_order: clean store_name (left of ':') then map via ship_to to canonical store_name
  oo AS (
    SELECT
      o.sku_code,
