...
-- Recreate get_srr_data with po_group
...
BEGIN
  ...
  minmax_agg AS (
    SELECT mm.item_id,
      ...
      COALESCE(SUM(CASE WHEN mm.type_store IN ('Kokkok','Kokkok-FC') THEN mm.min_val END),0) AS min_kokkok,
      COALESCE(SUM(CASE WHEN mm.type_store IN ('Kokkok','Kokkok-FC') THEN mm.max_val END),0) AS max_kokkok,
...
-- Recreate get_srr_d2s_data with po_group
...
RETURNS TABLE(
  sku_code text, main_barcode text, product_name_la text, product_name_en text,
  vendor_code text, vendor_display_name text, spc_name text,
  order_day text, delivery_day text, trade_term text,
  rank_sales text, leadtime numeric, order_cycle numeric,
