...
RETURNS TABLE(
  ...
  order_cycle numeric, store_name text, type_store text, min_store numeric,
  max_store numeric, stock_store numeric, stock_dc numeric, avg_sales_store numeric,
...
AS $function$
  ...
  -- Items without any minmax entry
  no_minmax_items AS (
    SELECT bi.bi_sku AS ms_sku, '(No Min/Max)'::text AS ms_store, ''::text AS ms_type
    FROM base_items bi
  ...
  -- Map type_store for stock/sales aggregation
  type_mapping AS (
