...
AS $function$
  ...
  mm AS (
    -- Per-store min/max  (join key: minmax.item_id = sku_code, minmax.store_name = store_type.store_name)
  ...
  st_store AS (
    -- FIX: sum by store (stock.company = store_type.store_name) ไม่ใช่ type_store
