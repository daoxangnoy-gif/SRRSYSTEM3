...
AS $function$
  ...
  -- Stores from minmax table for these SKUs (excluding Kokkok-FC)
  minmax_pairs AS (
    SELECT DISTINCT m.item_id AS mp_sku, m.store_name AS mp_store,
                    COALESCE(m.type_store, '') AS mp_type
    FROM minmax m
    WHERE m.item_id IN (SELECT bi_sku FROM base_items)
      AND m.store_name IS NOT NULL
      AND m.store_name <> ''
      AND (m.type_store IS NULL OR m.type_store != 'Kokkok-FC')
  ),
  -- Default store list (every store in store_type, excluding DC and Kokkok-FC)
  default_stores AS (
    SELECT DISTINCT ON (st.store_name)
      st.store_name AS ds_store,
      COALESCE(st.type_store, '') AS ds_type
    FROM store_type st
  ...
  -- UNION: pairs from minmax + every (SKU x default_store) for SKUs that have NO minmax record at all
  skus_without_minmax AS (
    SELECT bi.bi_sku FROM base_items bi
    WHERE bi.bi_sku NOT IN (SELECT mp_sku FROM minmax_pairs)
  ),
  store_items AS (
    SELECT mp_sku AS ms_sku, mp_store AS ms_store, mp_type AS ms_type
    FROM minmax_pairs
    UNION
    SELECT swm.bi_sku, ds.ds_store, ds.ds_type
    FROM skus_without_minmax swm
    CROSS JOIN default_stores ds
  ),
  -- Min/Max from minmax table directly (fast, indexed)
  minmax_agg AS (
    SELECT m.item_id AS mm_item, m.store_name AS mm_store,
      COALESCE(SUM(m.min_val),0) AS mm_min,
      COALESCE(SUM(m.max_val),0) AS mm_max
    FROM minmax m
    WHERE m.item_id IN (SELECT bi_sku FROM base_items)
      AND (m.type_store IS NULL OR m.type_store != 'Kokkok-FC')
