...
-- Fix get_srr_data: ใช้ COALESCE(item_id, id18) เพื่อ join sales_by_week และ on_order
...
AS $function$
  WITH vm AS (
    SELECT DISTINCT ON (vendor_code)
      vendor_code, spc_name, order_day, leadtime, order_cycle, supplier_currency
    FROM vendor_master
  ...
  pc AS (
    ...
    FROM po_cost WHERE item_id IS NOT NULL
    ORDER BY item_id, updated_at DESC
  ),
  mm AS (
    ...
    FROM (SELECT item_id, NULL::text AS id18, type_store, min_val, max_val FROM minmax) m
    WHERE COALESCE(item_id, id18) IS NOT NULL
    GROUP BY COALESCE(item_id, id18)
  ),
  st AS (
    SELECT COALESCE(item_id, NULL) AS item_id,
    ...
    FROM stock WHERE item_id IS NOT NULL GROUP BY item_id
  ),
  sw AS (
    SELECT COALESCE(item_id, id18) AS item_id,
      ...
      AVG(CASE WHEN type_store='UDEE' THEN avg_day END) AS avg_sales_udee
    FROM sales_by_week
    WHERE COALESCE(item_id, id18) IS NOT NULL
    GROUP BY COALESCE(item_id, id18)
  ),
  oo AS (
    SELECT COALESCE(item_id, id18) AS item_id, SUM(COALESCE(po_qty,0)) AS on_order
    FROM on_order
    WHERE COALESCE(item_id, id18) IS NOT NULL
    GROUP BY COALESCE(item_id, id18)
  )
  ...
  LEFT JOIN oo ON oo.item_id = dm.sku_code;
$function$;

-- Fix get_srr_d2s_data: ใช้ COALESCE(item_id, id18) สำหรับ sales_by_week และ on_order
...
AS $function$
  ...
  sw AS (
    SELECT COALESCE(item_id, id18) AS item_id, store_name, type_store, AVG(avg_day) AS avg_sales_store
    FROM sales_by_week
    WHERE COALESCE(item_id, id18) IN (SELECT sku_code FROM sku_set)
      AND store_name IS NOT NULL
      AND (type_store IS NULL OR type_store <> 'Kokkok-Fc')
    GROUP BY COALESCE(item_id, id18), store_name, type_store
  ),
  oo AS (
    SELECT COALESCE(item_id, id18) AS item_id, store_name, SUM(COALESCE(po_qty,0)) AS on_order_store
    FROM on_order
    WHERE COALESCE(item_id, id18) IN (SELECT sku_code FROM sku_set) AND store_name IS NOT NULL
