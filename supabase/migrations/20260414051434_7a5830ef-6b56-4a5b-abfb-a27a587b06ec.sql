-- Drop the old 3-param overload to avoid confusion
DROP FUNCTION IF EXISTS public.get_srr_data(text[], text[], text[]);

-- Recreate optimized 4-param version using CTE bulk approach
...
RETURNS TABLE(
  ...
  min_udee numeric, max_udee numeric,
  stock_dc numeric, stock_jmart numeric, stock_kokkok numeric, stock_udee numeric,
  avg_sales_jmart numeric, avg_sales_kokkok numeric, avg_sales_udee numeric,
  moq numeric, po_cost numeric, po_cost_unit numeric, on_order numeric
)
LANGUAGE plpgsql
SECURITY DEFINER
...
-- Create indexes to speed up the CTE subqueries
CREATE INDEX IF NOT EXISTS idx_data_master_filters ON data_master(sku_code, packing_size_qty, stock_unit_flag, buying_status, product_owner, item_type);
CREATE INDEX IF NOT EXISTS idx_rank_sales_item ON rank_sales(item_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_minmax_item ON minmax(item_id, type_store);
CREATE INDEX IF NOT EXISTS idx_stock_item ON stock(item_id, type_store);
CREATE INDEX IF NOT EXISTS idx_sales_week_id18 ON sales_by_week(id18, type_store);
CREATE INDEX IF NOT EXISTS idx_po_cost_item ON po_cost(item_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_on_order_sku ON on_order(sku_code);
