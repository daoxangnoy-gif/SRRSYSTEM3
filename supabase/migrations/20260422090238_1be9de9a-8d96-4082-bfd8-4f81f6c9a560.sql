...
AS $function$
  ...
  sw AS (
    SELECT COALESCE(item_id, id18) AS item_id,
      SUM(CASE WHEN type_store='Jmart' THEN COALESCE(avg_day,0) END) AS avg_sales_jmart,
      SUM(CASE WHEN type_store IN ('Kokkok','Kokkok-FC') THEN COALESCE(avg_day,0) END) AS avg_sales_kokkok,
      SUM(CASE WHEN type_store='U-dee' THEN COALESCE(avg_day,0) END) AS avg_sales_udee
    FROM sales_by_week
