...
AS $function$
  ...
  mm AS (
    SELECT item_id,
      ...
      MAX(CASE WHEN type_store='U-dee' THEN min_val END) AS min_udee,
      MAX(CASE WHEN type_store='U-dee' THEN max_val END) AS max_udee
    FROM minmax
...
-- ============ SRR DIRECT (D2S) - FIXED on_order suffix ============
