...
-- 1. Add store_name column to store_type
ALTER TABLE public.store_type ADD COLUMN IF NOT EXISTS store_name text;

-- 2. Update get_srr_d2s_data: No MinMax items gen to ALL stores
...
RETURNS TABLE(
  ...
  delivery_day text, trade_term text, rank_sales text, leadtime numeric, order_cycle numeric,
  store_name text, type_store text, min_store numeric, max_store numeric,
  stock_store numeric, stock_dc numeric, avg_sales_store numeric,
  moq numeric, po_cost numeric, po_cost_unit numeric, on_order_store numeric,
  unit_of_measure text, item_type text, buying_status text
)
LANGUAGE sql
SECURITY DEFINER
...
AS $function$
  ...
  -- Items without any minmax entry => generate for ALL stores
  no_minmax_items AS (
  ...
  -- Combined: all items with store assignments
  all_store_items AS (
  ...
  type_mapping AS (
    SELECT asi.ms_sku, asi.ms_store, asi.ms_type,
      CASE
        WHEN asi.ms_type IN ('Kokkok','Kokkok-FC') THEN 'Kokkok'
        ELSE asi.ms_type
      END AS mapped_type
  ...
  JOIN base_items bi ON bi.bi_sku = tm.ms_sku
  LEFT JOIN filtered_vendors fv ON bi.bi_vendor = fv.vc
  LEFT JOIN rank_data rd ON rd.rd_item = bi.bi_sku
  LEFT JOIN minmax_agg ma ON ma.mm_item = bi.bi_sku AND ma.mm_store = tm.ms_store
  ...
  LEFT JOIN order_store_agg os ON os.os_item = bi.bi_sku AND os.os_store = tm.ms_store
  ORDER BY bi.bi_vendor, bi.bi_sku, tm.ms_store;
$function$;
